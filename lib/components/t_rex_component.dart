...
import 'package:minigame/components/enemy_component.dart';
import 'package:minigame/constant/animation_state.dart';
import 'package:minigame/constant/constants.dart';
import 'package:minigame/manager/manager.dart';
import 'package:minigame/models/player_model.dart';

class TRexComponent extends SpriteAnimationGroupComponent<TRexAnimationState> with CollisionCallbacks, HasGameRef<GameManager> {
  TRexComponent(Image image, this.playerData) : super.fromFrameData(image, AssetManager.animation);
  ...
  void update(double dt) {
    ...
    if (isOnGround) {
      y = yMax;
      speedY = 0.0;
      if ((current != TRexAnimationState.hit) && (current != TRexAnimationState.run)) {
        current = TRexAnimationState.run;
      }
    }
